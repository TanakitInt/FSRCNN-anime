...
def prepare_data(_path):
    ...
    for i in range(nums):
        ...
        lr_img = cv2.resize(hr_img, (int(shape[1] / scale), int(shape[0] / scale)), interpolation = cv2.INTER_CUBIC)
...
def prepare_crop_data(_path):
    ...
    for i in range(nums):
        ...
        # two resize operation to produce training data and labels
        lr_img = cv2.resize(hr_img, (int(new_width), int(new_height)), interpolation = cv2.INTER_CUBIC)
